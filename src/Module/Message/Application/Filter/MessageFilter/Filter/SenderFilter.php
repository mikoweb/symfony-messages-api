...
readonly class SenderFilter implements FilterInterface
{
    ...
    {
        return !is_null($query->sender);
    }

    public function apply(Builder $queryBuilder, MessageQueryDto $query): void
    {
        /** @var Collection<PersonDto> $senders */
        $senders = $this->queryBus->dispatch(new FindPersonCollectionQuery($query->sender));
        $queryBuilder->field('senderId')->in(array_unique($senders->column('id')));
    }
}
